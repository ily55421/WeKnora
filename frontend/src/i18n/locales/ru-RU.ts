export default {
  ...
  knowledgeEditor: {
    ...
    rebuildLinks: 'Перестроить ссылки страниц',
    rebuildLinksDone: 'Ссылки страниц перестроены',
    rebuildLinksFailed: 'Не удалось перестроить ссылки страниц',
    ...
    indexing: {
      ...
      rebuildFailed: 'Не удалось перестроить индекс. Повторите попытку позже',
  ...
  knowledgeList: {
    ...
    messages: {
      ...
      copyStarted: 'Копирование базы знаний начато (вместе с содержимым). Прогресс отображается в списке.',
      copyProgress: 'Копирование базы знаний {percent}% ({processed}/{total})',
      copySuccess: 'База знаний скопирована',
      copyFailed: 'Не удалось скопировать базу знаний',
      copyInProgress: 'Уже выполняется другая задача копирования. Дождитесь её завершения.',
      copyTimeout: 'Копирование идёт слишком долго. Отслеживание прогресса остановлено — проверьте результат в списке позже.',
    },
    sections: {
      pinned: 'Закреплённые',
      mine: 'Созданные мной',
    ...
    menu: {
      ...
      copyWithContent: 'Копировать с содержимым'
    },
    delete: {
  ...
  evaluation: {
    ...
    description: 'Запустите оценку качества поиска и генерации по базе знаний на фиксированном наборе и получите сопоставимые количественные метрики.',
    introTitle: 'Набор для оценки предоставляет сервер',
    introDescription: 'Для оценки используется встроенный в сервер набор QA (каталог dataset/samples в репозитории). Содержимое вашей базы знаний и её документы не читаются и не изменяются. Каждый запуск реально обращается к моделям чата и реранжирования, поэтому учитывайте стоимость.',
    kbLabel: 'База знаний для оценки',
    kbDescription: 'База знаний, которую нужно оценить. Поиск идёт по существующему индексу, документы повторно не разбираются.',
    kbPlaceholder: 'Выберите базу знаний',
    chatModelLabel: 'Модель чата',
    chatModelDescription: 'Модель, генерирующая ответы. Напрямую влияет на метрики генерации (BLEU, ROUGE).',
    rerankModelLabel: 'Модель реранжирования',
    rerankModelDescription: 'Необязательно. Если указана, результаты поиска сначала реранжируются, что позволяет оценить выигрыш от реранжирования.',
    start: 'Запустить оценку',
    running: 'Оценка выполняется…',
    requiredHint: 'Сначала выберите базу знаний и модель чата',
    resultTitle: 'Результат оценки',
    taskId: 'ID задачи',
    runningHint: 'Оценка выполняется в фоне. Можно покинуть страницу и посмотреть результат позже.',
    failedFallback: 'Оценка завершилась ошибкой, сервер не вернул причину.',
    progressPending: 'Подготовка данных для оценки…',
    progressLabel: 'Готово {finished}/{total}',
    statusStarting: 'Отправка',
    statusPending: 'В очереди',
    statusRunning: 'Выполняется',
    statusSuccess: 'Завершено',
    statusFailed: 'Ошибка',
    retrievalMetrics: 'Метрики поиска',
    generationMetrics: 'Метрики генерации',
    metricPrecision: 'Точность Precision',
    metricRecall: 'Полнота Recall',
    metricNdcg3: 'NDCG (Top-3)',
    metricNdcg10: 'NDCG (Top-10)',
    metricMrr: 'MRR',
    metricMap: 'MAP',
    metricBleu1: 'BLEU-1',
    metricBleu2: 'BLEU-2',
    metricBleu4: 'BLEU-4',
    metricRouge1: 'ROUGE-1',
    metricRouge2: 'ROUGE-2',
    metricRougeL: 'ROUGE-L',
    toasts: {
      started: 'Задача оценки отправлена и выполняется в фоне',
      startFailed: 'Не удалось запустить оценку: {message}',
      success: 'Оценка завершена',
      failed: 'Оценка не удалась: {message}',
      loadKbFailed: 'Не удалось загрузить список баз знаний',
      pollTimeout: 'Оценка идёт слишком долго. Отслеживание прогресса остановлено — проверьте результат позже.',
      pollLost: 'Не удаётся получить прогресс оценки. Возможно, задача потеряна при перезапуске сервера — запустите оценку заново.'
    }
  },
  chatHistorySettings: {
